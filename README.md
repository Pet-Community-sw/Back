...
## 📐 아키텍처
![Image](https://github.com/user-attachments/assets/3c710d34-38fb-430b-a6cb-022405c1b552)
### ✅**Nginx 기반 리버스 프록시 구성**

- 백엔드 애플리케이션(Spring Boot)을 외부 도메인으로 안정적으로 노출하기 위해 Nginx를 리버스 프록시 서버로 구성합니다.
- 내부 포트(예: `localhost:8080`)로 서비스되는 애플리케이션을 외부 도메인 `petApp.com`으로 연결합니다.
...
앱이 실행 중인 사용자(Foreground 사용자)에게는 FCM이 아닌 SSE 기반의 Redis Pub/Sub 실시간 알림을 전송하고, 앱이 꺼져 있는 사용자에게는 FCM 푸시를 보낼 수 있도록 분기 처리된 알림 시스템을 설계하였습니다.
